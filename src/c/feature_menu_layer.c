...
void Event_Generator(MenuIndex *cell_index)
{
  ...
  {
    case 1:
    toast_layer_show(message_layer, "GIF PRICES HAVE BOTTOMED OUT!\n", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    ...
    case 3:
    toast_layer_show(message_layer, "HATERS ARE SELLING CHEAP SPONGEBOB MEMES FROM LAST WEEK'S RAID!\n", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    Player.Trenchcoat.Drug[HEROINE].Price = rand() % 1151 + 850;
    break;

    case 4:
    case 5:
    toast_layer_show(message_layer, "ADDICTS ARE BUYING SPONGEBOB MEMES AT OUTRAGEOUS PRICES!\n", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    Player.Trenchcoat.Drug[HEROINE].Price = rand() % 25001 + 18000;
    break;

    case 6:
    case 7:
    toast_layer_show(message_layer, "HATERS MADE A BIG DOGE BUST!\nPRICES ARE OUTRAGEOUS!\n", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    ...
    case 17:
    toast_layer_show(message_layer, "THE MARKET IS FLOODED WITH CHEAP HOME-MADE ODNS!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    Player.Trenchcoat.Drug[ACID].Price = (rand() % 51 + 20) * 10;
    break;	

    case 18:
    toast_layer_show(message_layer, "LUCKY YOU! YOU CAUGHT SOME FAST INTERNET AND SAVED A DAY!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    Player.Day--;
    break;	
  }
}
...
static void menu_draw_header_callback(GContext* ctx, const Layer *cell_layer, uint16_t section_index, void *data)
{
  ...
  switch(Player.MenuNumber)
  {
    ...
    case 0:
    if (Player.Day == 1)
      menu_header_simple_icon_draw(ctx, cell_layer, "MEMEWARS", game_icon);    
    ...
    switch (Player.MenuNumber)
    {
      ...
      if (cell_index->row < 7)
      {
        if (Player.Trenchcoat.Freespace < 1)
          toast_layer_show(message_layer, "YOU'RE GONNA NEED MORE PHONE STORAGE!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
        else if (Player.Money.Cash < Player.Trenchcoat.Drug[cell_index->row].Price)
          toast_layer_show(message_layer, "YOU CAN'T AFFORD THOSE MEMES!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
          else
        {
      ...
      {
        toast_layer_show(message_layer, "THE MEME SHARK IS IN TEH INTERWEBZ", LONG_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
      ...
      if (Player.CurrentCity != BROOKLYN)
        toast_layer_show(message_layer, "THE BANK IS IN MEMELYN!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
      else
      {
        if (cell_index->row == 1)
      ...
      if (cell_index->row == 4)
      {
        if (Player.Trenchcoat.Guns[0].Quantity == 0)
          toast_layer_show(message_layer, "YOU DON'T HAVE ANY SERVICES!\nYOU HAVE TO RUN!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
        else if (Player.Trenchcoat.Guns[0].Ammo == 0)
          toast_layer_show(message_layer, "YOU DON'T HAVE ANY RECEPTION!\nYOU HAVE TO RUN!", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);					
        else
        {
          for(X = 1; X < 4; X++) {  
      ...
      if (Player.Cops < 1 && Player.Money.Cash >= 1200 && Player.Damage > 0)
      {
        ...
        confirm_header = (char*)malloc((strlen("WILL YOU PAY $50000 FOR A MEME DOCTOR TO SEW YOU UP?") + 1) * sizeof(char));
        snprintf(confirm_header,
                 (strlen("WILL YOU PAY $50000 FOR A MEME DOCTOR TO SEW YOU UP?") + 1) * sizeof(char),
                 "WILL YOU PAY $%i FOR A MEME DOCTOR TO SEW YOU UP?",
                 max_meds);
        p_MenuCallbackContext[0] = NULL;
...
{
  if (Settings.vibrate) vibes_short_pulse();
  toast_layer_show(message_layer, "YOU GET HIGH ON MEMES, STUMBLE ON TO SOME TRACKS AND GET HIT BY A TRAIN!", LONG_MESSAGE_DELAY, menu_header_heights[0]);
  safe_free(confirm_header);
...
void Cop_187(MenuIndex *cell_index)
{
  ...
  X = (rand() % 1250 + 750);
  Player.Money.Cash += X;
  string = (char*)malloc((strlen("\nHOLY SHIT! YOU KILLED ALL OF THEM AND FOUND $2000 ON OFFICER MEMEHATER!") + 1) * sizeof(char));
  if (Player.Dice == 9)
    ...
    snprintf(string,  (strlen("\nHOLY SHIT! YOU KILLED ALL OF THEM AND FOUND $2000 ON OFFICER MEMEHATER!") + 1) * sizeof(char), "\nHOLY SHIT! YOU KILLED ALL OF THEM AND FOUND $%u ON OFFICER MEMEHATER!", X);
